...
function externalTotalsRowToSeason(row = {}) {
  ...
  return {
    ...
    fgm: perGame(row.FG, gp),
    fga: perGame(row.FGA, gp),
    tpm: perGame(row['3P'], gp),
    tpa: perGame(row['3PA'], gp),
    ftm: perGame(row.FT, gp),
    fta: perGame(row.FTA, gp),
...
function externalPerGameRowToSeason(row = {}, seasonEndYear = 2025) {
  ...
  return {
    ...
    fgm: roundStat(row.FG, 1),
    fga: roundStat(row.FGA, 1),
    tpm: roundStat(row['3P'], 1),
    tpa: roundStat(row['3PA'], 1),
    ftm: roundStat(row.FT, 1),
    fta: roundStat(row.FTA, 1),
    fgPct: pctFromStat(row['FG%']),
    tpPct: pctFromStat(row['3P%']),
    ftPct: pctFromStat(row['FT%']),
...
function boxAggregateToSeasonRow(acc) {
  ...
  return {
    ...
    fgm: roundStat(acc.fgm / acc.gp, 1),
    fga: roundStat(acc.fga / acc.gp, 1),
    tpm: roundStat(acc.tpm / acc.gp, 1),
    tpa: roundStat(acc.tpa / acc.gp, 1),
    ftm: roundStat(acc.ftm / acc.gp, 1),
    fta: roundStat(acc.fta / acc.gp, 1),
