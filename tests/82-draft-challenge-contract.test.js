...
assert.ok(sim.includes('const POSSESSION_FT_WEIGHT = 0.44'), 'league row simulation should centralize the possession free-throw weight');
assert.ok(sim.includes('const MAX_PLAYER_3PA_GAME = 16'), 'league row simulation should cap impossible individual three-point attempt games');
assert.ok(sim.includes('function realFieldGoalAttemptsPerGameForSim'), 'league row simulation should read real source-season FGA volume when available');
assert.ok(sim.includes('function realTurnoversPerGameForSim'), 'league row simulation should read real source-season turnovers when available');
assert.ok(sim.includes('function realThreePctForSim'), 'league row simulation should read real source-season 3P% for expected efficiency');
assert.ok(sim.includes('function buildPlayerOffensivePriorForSim'), 'league row simulation should build player offensive priors before allocating usage');
assert.ok(sim.includes('function capAllocatedShares'), 'league row simulation should redistribute usage and shots without exceeding player caps');
assert.ok(sim.includes('function expectedThreePctForVolume'), 'league row simulation should calculate 3P% after final 3PA volume is known');
assert.ok(sim.includes('const exactMakes = attempts * pct'), 'three-point makes should use unbiased expected-make rounding for low-volume shooters');
assert.ok(sim.includes('const makeBump = Math.random() < fraction ? 1 : 0'), 'three-point makes should preserve fractional expected makes across a season');
assert.ok(sim.includes('usageShare'), 'shot plans should expose player usage share diagnostics');
assert.ok(sim.includes('comfortUsageRate'), 'shot plans should expose player comfort usage diagnostics');
assert.ok(sim.includes('tpaCapReason'), 'shot plans should explain why a three-point attempt cap applied');
assert.ok(sim.includes('volumePenalty'), 'shot plans should expose high-volume three-point efficiency penalties');
assert.ok(sim.includes('expectedThreePct'), 'shot plans should expose final expected three-point percentage');
assert.ok(sim.includes('capHit'), 'shot plans should expose three-point cap hit diagnostics');
...
assert.ok(sim.includes('const targetThreePct = clamp(parseNum(shotPlan.expectedThreePct'), 'player box rows should consume shotPlan.expectedThreePct after final 3PA allocation');
assert.ok(sim.includes('function applyReconcilePointDeltaToLine'), 'team point reconciliation should preserve allocated threes unless it has no safer correction');
assert.ok(sim.includes('applyReconcilePointDeltaToLine(row, step)'), 'team point reconciliation should use the no-extra-threes correction path');
...
assert.ok(script.includes('高负荷降效'), 'challenge result tags should explain high-volume shooting efficiency penalties');
assert.ok(script.includes('三分封顶'), 'challenge result tags should explain repeated three-point cap hits');
assert.ok(script.includes('avgVolumePenalty'), 'challenge selected stats should expose average volume penalty diagnostics');
assert.ok(script.includes('capHitCount'), 'challenge selected stats should expose three-point cap hit counts');
